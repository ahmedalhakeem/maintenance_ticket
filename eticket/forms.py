...
class Ticket_Form(forms.Form):
    tour_type = forms.ChoiceField(required=True,label='نوع الجولة', widget=forms.Select(attrs={'class':'form-control-lg', 'id':"form12",}), choices=[('داخلية', 'داخلية'),('خارجية','خارجية')])
    tour_name = forms.CharField( required=True,label='عنوان الجولة', widget=forms.TextInput(attrs={'class': 'form-control-lg'}))
    tour_date = forms.DateField(required=True,label='تاريخ بدايةالجولة', widget=widgets.DateInput(attrs={'class': 'form-control-lg','type':'date', 'id':"start"}))
    tour_duration = forms.IntegerField(required=True,label='عدد ايام الجولة', widget=widgets.NumberInput(attrs={'class':'form-control-lg', 'id':'tour-days', "lang":"ar_SA" }))  
    ...
    expected_end_tour = forms.DateField(required=False,label='تاريخ نهاية الجولة المتوقع', widget=widgets.DateInput(attrs={'type': 'date','class': 'form-control-lg', 'id': 'end'}))
